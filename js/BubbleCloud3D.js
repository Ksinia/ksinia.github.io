...
function Bubble(vector, content, settings, centralBubble) {
    this.vector = vector;
    ...
    var sizePercentage = 0.45; //Math.max((settings.origin.z + this.vector.z) / settings.frameDepth, 0.2);
    if (centralBubble){
        sizePercentage = 0.8;
    }
    var size = (settings.frameWidth / 2) * sizePercentage;
...
Bubble.prototype.giveAngularRotationOnAxis = function(x, y) {
    ...
    if (this.changed == false && this.vector.z > 223){
        this.changed = true;
        this.element.html(jQuery(this.content).data('ref'));
        jQuery("#popup").html(jQuery(this.content).data('text'));
        jQuery("#popup").modal();
    }
    if (this.changed == true && this.vector.z <=223){
        this.changed = false;
